...
- 认证框架：使用`spring security`。
- 缓存方案：使用`redis`存储token。
- 密码传递：使用`RSA`加密明文密码后，传输到服务器，在服务器上解密为明文。RSA密钥对在`resources/application.properties`中配置。注意这个密钥对与[rbac-lite-vue](https://github.com/basicworld/rbac-lite-vue)前端项目对应的密钥对必须一致。
- 密码存储：使用`BCrypt`加密明文密码。
- 验证码：使用算数验证码，验证码有效时间为5分钟，存储在redis中，到期自动失效，验证码的图片格式是jpg。
- 前端路由加载：用户登录后，从后台生成用户路由信息，传递到前台。

### 权限设计

- 权限具体到页面级别。
- 每个角色可以拥有多个权限，即访问多个页面。
- 每个用户可以拥有多个角色，用户的权限（可访问页面）是所有角色关联页面的合集。
- 不设计角色组、不设计部门权限。用户信息中的“部门”只作为个人信息显示，与权限无关。
- 登录用户的信息存储在redis中，用户的权限在本次登录后一直有效，直到退出登录。修改用户权限对本次登录无效。
- 修改用户权限，会在用户下次登录时生效。
- 超级管理员：超级管理员的权限通过“关联超级管理员角色”来配置。系统默认有一个角色“超级管理员”，角色代码为admin。关联这个角色的用户视为超级管理员。即系统可以有多个超级管理员。
- 只能由超级管理员来创建、修改、删除其他的超级管理员。非超级管理员不能修改、删除、创建、启用、停用超级管理员账号。
- 不限制多个终端登录。同一个账号可以在多个地方登录。

## 邮箱功能说明

163邮箱建议使用smtp 994端口，并开启SSL加密。经测试994比465更稳定，465易出现连接失败（北京移动网络）。
...
- 通过header中的token来传递用户身份信息。匿名访问的api不需要token，非匿名api需要token。
